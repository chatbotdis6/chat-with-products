...
class DataTransformer:
    ...
    @staticmethod
    def productos_to_proveedores(productos: List[ProductoInfo]) -> List[ProveedorInfo]:
        """
        Agrupa productos por proveedor y genera metadata de proveedores.
        
        Args:
            productos: Lista de productos
            
        Returns:
            Lista de proveedores con información agregada
        """
        ...
        for p in productos:
            ...
            # Completa faltantes
            if not prov["ejecutivo_ventas"] and p["ejecutivo_ventas"]:
                prov["ejecutivo_ventas"] = p["ejecutivo_ventas"]
            if not prov["pagina_web"] and p["pagina_web"]:
                prov["pagina_web"] = p["pagina_web"]
            
            # Fusiona teléfonos/links sin duplicados
            for d in p["whatsapp_ventas_list"]:
                if d not in prov["whatsapp_ventas_list"]:
                    prov["whatsapp_ventas_list"].append(d)
            for l in p["whatsapp_links"]:
